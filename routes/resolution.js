          ...
          if (pageAmount) {
            ...
            resolutions
              ...
              .catch(
                ...
                err =>
                  logger.error("could not update page amount", {
                    stack: err.stack
                  })
              );
          }
  ...
  routingUtil.fullAuth(
      ...
      res.render("editor", getEditorViewParams(true, resDoc, token, codeDoc)),
    {
      permissionMissmatch: (token, resDoc, codeDoc) =>
        ...
        res.render(
          "editor",
          getEditorViewParams(false, resDoc, token, codeDoc)
        ),
...
router.post("/delete/:token", (req, res) => {
  ...
  resolutions
    ...
    .then(resDoc =>
      Promise.all([
        resolutionArchive.insertOne(resDoc),
        resolutions.deleteOne({ token })
      ])
    )
    .then(
      () => {
...
const padNumber = (number, amount) => {
  ...
  return amount < number.length
    ? number
    : "0".repeat(amount - number.length) + number;
};
  ...
  routingUtil.fullAuth(
    ...
    (token, resDoc) => {
      ...
      for important question to pass*/
        voteResults.passed =
          voteResults.inFavor >
          voteResults.against * (voteResults.importantQuestion ? 2 : 1);
        ...
        issueError(
          ...
          res,
          400,
          "got no vote results although they were required"
        );
        return;
      }
