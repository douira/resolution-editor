...
function registerEventHandlers(loadedData) {
  ...
  if (accessLevel === "MA" ||
    ...
    //the current selected clause (in a list), start with none
    ...
    var updateAmd = function() {
      ...
      if (amdActionType === "add" || amdActionType === "replace") {
        ...
        var amdClauseSelection = amdClauseListSelection.children(".clause").trigger("reset");

        //if add type
        if (amdActionType === "add") {
          //set the clause number to one more than there are op clauses
          amdClauseSelection
            .children("h6")
            .find(".clause-number")
    ...
    $("#amd-type-select-box").on("change", function() {
      //update action type
      amdActionType = $(this).getSelectValueId();
    ...
    .on("click", function(e) {
      ...
      if (amdOrigClause.get(0) !== clause.get(0)) {
        ...
        //to prevent amd display reset occuring although that clause isn't selected anymore
