...
//sets the value id of a select container (like the above)
$.fn.setSelectValueId = function(setValueId) {
  //get the actual wrapper element generated by materialize
  var selectWrapper = $(this).find(".select-wrapper");

  //remove the activation from any element
  selectWrapper.children("li").removeClass("active selected");
  console.log("setvalue", setValueId);
  //add activation classes to li element at the index that has a matching element with the
  //target select value in the select element
  selectWrapper
    .children("li")
    .eq(selectWrapper.find("option[value='" + setValueId + "']").index())
...
function registerEventHandlers(loadedData) {
  ...
  if (accessLevel === "MA" ||
    ...
    /*TODO:
    what happens when add is presently selected and a clause is then selected
    make sure nothing bad happens when add is selected and any clause is (re)moved
    clause is present and add is then selected?
    ...
    */
    ...
    getAmendmentUpdate = function(noData) {
      console.log("update on structure");
