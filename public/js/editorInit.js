...
$.fn.clauseRemovable = function() {
  return this.parent().children(".clause").length >= 2 || this.isSubClause();
};
...
$.fn.canReceiveSubclause = function() {
  ...
  return (
    this.amountAbove(".clause-list-sub") <
    allowedSubclauseDepth[this.closest(".clause").attr("data-clause-type")]
  );
};
...
$.fn.cleanupClonedTooltips = function() {
  ...
  this.find(".tooltipped")
    .removeAttr("data-tooltip-id")
    .tooltip();
...
$.fn.addSubClause = function(activationStateChanges) {
  ...
  if (!subList.length) {
    ...
    subList = this.closest(".clause-list")
      .clone(true, true)
  ...
  strippedClause.children(".phrase-input-wrapper").remove();

  //also remove phrase condensed field
  strippedClause
    .children(".clause-cond")
    .children(".cond-phrase")
    .remove();
...
$.fn.addClause = function(amount, activationStateChanges) {
  ...
  if (!this.is(".add-clause-container")) {
    if (this.is(".clause-list")) {
      ...
      log({
        msg: "could not find add clause container for element",
        elem: this
      });
      return this;
    }
...
const registerEventHandlers = loadedData => {
  ...
  if (allowLV) {
    ...
    const updateAmd = sendNoUpdate => {
      ...
      if (amdActionType === "add") {
        ...
        amdOrigClause = $("#op-clauses")
          .children(".clause-list")
          .children(".clause")
          .eq(0);
    ...
    const saveAmd = saveType => {
      ...
      if (!saveAmdUpdate) {
        ...
        console.error(
          "Got falsy amd update object while doing saveAmd update!"
        );
        return;
      }
      ...
      if (saveType === "apply") {
        ...
        if (amdActionType === "remove") {
          ...
          $("#op-clauses > .clause-list > .add-clause-container")
            .before(amdClauseElem)
          ...
          console.error(
            "invalid action type found when doing saveAmd",
            amdActionType
          );
        }
    ...
    amdTypeSelect.on("change", function() {
      ...
      if (
        ["noselection", "add", "change", "replace", "remove"].includes(activeId)
      ) {
        //got active id
      ...
      selectBoxSubmitButton
        .on("click", e => {
          ...
          if (selectedValue) {
            ...
            selectForm.append(
              `<input type='hidden' name='code' value='${resolutionCode}'>`
            );

            //add action url to form with token
            selectForm.attr(
              "action",
              `/resolution/setattribs/${resolutionToken}`
            );
  ...
  $(".autocomplete").on("init", function(e) {
    e.stopPropagation();
    const elem = $(this);
    ...
    if (foundData) {
      ...
      //there was no data for this element, error
      log({ msg: "no autocomplete data found for field", elem: this });
    }
  });

  //forum change handler
  $("input#forum-name").on("change", function(e) {
    e.stopPropagation();
  ...
  $("input:not(.not-editor *)").on("reset", function(e) {
    e.stopPropagation();
    $(this)
      .val("")
      .resetSiblingLabels();
  ...
  function chipsInit(e) {
    ...
    const initData =
      (chipsInstance && chipsInstance.chipsData) || elem.getData().initData;
    if (initData) {
  ...
  requiredContainers.on("checkRequired", ".chips.required", function() {
    const elem = $(this);
    ...
    if (!valueBad) {
      ...
      const mainSponsor = $("#main-spon")
        .val()
        .trim();
      valueBad =
        !value.reduce((allOk, item, index) => {
          ...
          elem
            .children(`.chip:eq(${index})`)
            .classState(!isOk, "red white-text");
  ...
  $(".clause")
    ...
    .on("editInactive", function(e, amdUpdatePossible, noExtCondUpdate) {
      ...
      if (
        !noExtCondUpdate &&
        !contentExtVal.length &&
        !clauseContentExt.hasClass("hide-this")
      ) {
      ...
      //hide input fields
      elem
        .children(".clause-content, .clause-content-ext, .phrase-input-wrapper")
        .setHide(true);
      ...
      if (phraseFieldWrapper.length) {
        ...
        condensedWrapper
          ...
          .html(
            processCondText(phraseField.val(), phraseField.hasClass("invalid"))
          );
      ...
      if (elem.clauseRemovable()) {
        ...
        if (parent.children(".clause").length === 1) {
          ...
          const extField = clause
            ...
            .setHide(true)
            .children("textarea");
          const contentField = clause
            .children(".clause-content")
            .children("textarea");
          contentField.val(
            `${contentField.val().trim()} ${extField.val().trim()}`.trim()
          );
  ...
  //the clause list
  $(".clause-list").on("fromLoadedData", function(e) {
    e.stopPropagation();
    const elem = $(this);
  ...
  $(".add-clause-container").on("click", function(e) {
    e.stopPropagation();
    const elem = $(this);
    ...
    .on("updateDisabled", function(e) {
      ...
      $(this).disabledState(
        !clause.find(".clause").length || //disable if doesn't already have subclause
          clause.children(".clause-content-ext:visible").length
  ...
  $("#delete-action-confirm").on("click", e => {
    e.stopPropagation();

    //send the delete request and go back to front page
    $.post(
      `/resolution/delete/${resolutionToken}`,
      { code: resolutionCode },
      () => (location.href = "/")
    );
  });
...
$(document).ready(() => {
  ...
  resolutionAttributes = {};
  ["readonly", "noadvance", "static"].forEach(
    name => (resolutionAttributes[name] = name === attributesString)
  );
  ...
  resolutionAttributes.allowSave =
    !resolutionAttributes.readonly || accessLevel === "MA";
  resolutionAttributes.allowAdvance =
    !resolutionAttributes.noadvance || accessLevel === "MA";

  //if chair, sg or master access, liveview allowed
  allowLV =
    (!resolutionAttributes.readonly &&
      ((resolutionStage === 6 &&
        (accessLevel === "CH" || accessLevel === "SG")) ||
  ...
  if ($("#advance-code-input")[0]) {
    ...
    registerAccessInputs(
      ...
      "#advance-code-form",
      {
        ...
        additionalInputsSelectors:
          "#infavor-vote-input,#against-vote-input,#abstention-vote-input"
      }
    );
  }
    ...
    $.getJSON("/extData")
      ...
      .done(extData => {
        ...
        for (const forumId in extData.forumsFlat) {
          ...
          loadedData.forumMapping[abbrevMappingPrep(forum.abbr)] = {
            to: forum.name
          };
        ...
        loadedData.generateAutofillData = selectedForum => {
          ...
          forumCountries.forEach(countryId => {
            ...
            loadedData.countryMapping[
              abbrevMappingPrep(country.placardname)
            ] = { to: country.name };
            loadedData.countryMapping[abbrevMappingPrep(country.fullname)] = {
              to: country.name
            };
          ...
          if (loadedData.scForum !== scForum) {
            ...
            loadedData.opPhrasesConverted = scForum
              ? opPhrasesConverted.sc
              : opPhrasesConverted.regular;
        ...
        loadedData.autofillInitData = {
          ...
          "#preamb-clauses .phrase-input": convertPropObj(
            loadedData.phrases.preamb
          ),
          "#op-clauses .phrase-input,#amd-clause-wrapper .phrase-input":
            "opPhrasesConverted"
        ...
        if (resolutionStage) {
          ...
          serverLoad(resolutionToken, true, initForum => {
            ...
            $(".clause input, .clause textarea").on("paste keyup", function() {
              //send content update
        ...
        if (
          ...
          !localStorage.getItem("helppagehint")
        ) {
