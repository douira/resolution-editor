 ...
 * http://www.opensource.org/licenses/mit-license.php
 Slightly adapted for resolution-editor by https://github.com/douira 2017 under the
 MIT license of the resolution-editor project
 *
 * @date 8 Jan 2013
...
(function($) {
  $.fn.scrollIntoView = function(duration, easing, complete) {
    ...
    var elY = Infinity,
      elH = 0;
    if (this.length == 1)
      (elY = this.get(0).offsetTop) == null ||
        (elH = elY + this.get(0).offsetHeight);
    else
      this.each(function(i, el) {
        el.offsetTop < elY
          ? (elY = el.offsetTop)
          : el.offsetTop + el.offsetHeight > elH
    ...
    while (pEl) {
      ...
      if (
        // it wiggles?
        (pEl.scrollTop != ((pEl.scrollTop += 1) == null || pEl.scrollTop) &&
          (pEl.scrollTop -= 1) != null) ||
        (pEl.scrollTop != ((pEl.scrollTop -= 1) == null || pEl.scrollTop) &&
          (pEl.scrollTop += 1) != null)
      ) {
    ...
    function scrollTo(el, scrollTo) {
      if (scrollTo === undefined) {
        ...
        $(el)
          .stop()
          .animate({ scrollTop: scrollTo }, opts);
      } else {
