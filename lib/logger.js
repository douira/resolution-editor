...
//add console output transport in dev mode
if (devEnv) {
  ...
  logger.add(new winston.transports.Console({
    ...
    format: winston.format.combine(
      ...
      //partially took inspiration from
      ...
      winston.format(function(info) {
        ...
        const contentSuffix = JSON.stringify(Object.assign({}, info, {
        ...
        info[MESSAGE] = `${timePrefix}${info.level}: ${info.message}` +
...
logger.exceptions.handle(
  new winston.transports.File({ filename: "./logs/events/exceptions.log" })
);
