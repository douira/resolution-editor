...
//add console logging serializer in dev mode
if (devEnv) {
  //add serializer to logger
  logger.addSerializers({
    err: err => {
      //log error for better viewing
      console.log(err);
  ...
  //use the normal standard error serializer
  logger.addSerializers({ err: bunyan.stdSerializers.err });
}
...
function issueError(req, res, status, msg, ...otherArgs) {
  //log to console in dev mode
  if (devEnv) {
    console.log(status, msg, ...otherArgs);
  }

  //log error if there was one
  if (otherArgs.length) {
    //also log other arguments passed
    req.log.error(otherArgs[0], msg, otherArgs.length > 1 && { other: otherArgs });
  } else {
    //log message only
    req.log.error(msg);
  }
  ...
  if (typeof res === "object") {
    ...
    try {
      ...
      logger.error(
        err, "critical: error logging other error because of duplicate response header set");
    }
  } //pass falsy value as res to prevent sending of error to client
}

//replace error logger with proxy in dev mode
if (devEnv) {
  //replace with proxy
  logger.error = new Proxy(logger.error, {
...
module.exports = { logger: logger, applyLoggerMiddleware, issueError};
