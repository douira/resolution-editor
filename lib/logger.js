...
const accessLogStream = rfs(
  "access.log",
  Object.assign({ path: loggingDir.access }, logStreamConfig)
);
  ...
  logger.add(
    new winston.transports.Console({
      ...
      format: winston.format.combine(
        ...
        //pass nice timestamp formatting form
        winston.format.timestamp(timestampFormat),
        ...
        winston.format(info => {
          ...
          //start to build message here, info object is trimmed down afterwards
          const messageBuild = `${timePrefix}${info.pid} ${info.level}: ${
            info.message
          }`;

          //remove options that should not be rendered
          delete info.level;
          delete info.message;
          delete info.splat;
