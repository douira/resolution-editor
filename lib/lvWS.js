  ...
  for (const prop in diff) {
    ...
    if (typeof value === "object") {
      ...
      processDiff(
        ...
        oldClausePart && oldClausePart[prop],
        newClausePart[prop]
      );
    } else {
      //value was deleted
...
const resolveChangePath = (
  prevObj,
  remainingPath,
  setValue,
  cache,
  dontReadCacheOrPathString
) => {
...
const dataToAllViewers = (tokenEntry, data) =>
  forEachObj(tokenEntry.viewers, v =>
    sendJson(v.socket, {
      type: data.type,
      update: data.update
    })
...
const processMessage = (clients, tokens, data, ws, accessToken, resDoc) => {
  ...
  switch (data.type) {
    ...
    case "amendment": {
      ...
      let oldClause =
        tokenEntry.latestStructure.resolution.clauses.operative[
          currentAmd.clauseIndex
        ];
    ...
    case "saveAmd": {
      ...
      if (
        ...
        !(
          ...
          ((typeof saveAmdUpdate.newClause === "object" &&
            saveAmdUpdate.newClause.phrase) ||
            saveAmdUpdate.type === "remove") &&
          //validate presence of clauseIndex if necessary
          (typeof saveAmdUpdate.clauseIndex === "number" ||
            saveAmdUpdate.type === "add") &&
    ...
    case "updateContent": {
      ...
      if (updatePath[updatePath.length - 1] === "amendment") {
        ...
        if (!tokenEntry.amd) {
          //error and stop
          logger.error(
            "received amendment content update but no amendment exists!"
          );
          return;
        }
      ...
      resolveChangePath(
        ...
        data.update.content,
        tokenEntry.pathCache
      ); //the cache built in previous content updates
  ...
  if (
    (data.type === "initEditor" || data.type === "initViewer") &&
    tokenEntry.editor &&
    Object.keys(tokenEntry.viewers)
  ) {
    //then update the liveview timestamp in the database for this resolution
    resolutions
      .updateOne(
        { token: clientEntry.token },
        { $set: { lastLiveview: Date.now() } }
      )
      .catch(err =>
        logger.error(err, "could not update last liveview timestamp")
      );
  }
};
  ...
  wss.on("connection", ws => {
      ...
      if (data.accessToken && data.accessToken === accessToken) {
        ...
        if (tokenProcessor.check(token)) {
          ...
          resolutions
              ...
              if (resDoc && tokenProcessor.check(code)) {
                ...
                access
                  ...
                  .then(codeDoc => {
                    ...
                    if (
                      codeDoc &&
                      resUtil.checkPermission(resDoc, codeDoc, "liveview")
                    ) {
                  ...
                  .catch(err =>
                    wsError(ws, "code db read error", { tryAgain: true, err })
                  );
              } else {
            ...
            .catch(err =>
              wsError(ws, "token db read error", { tryAgain: true, err })
            );
        } else {
    ...
    ws.on("close", () => {
      ...
      if (clientEntry.clientType === "viewer") {
        ...
        forEachObj(tokenEntry.viewers, v =>
          sendJson(v.socket, { type: "editorGone" })
        );
