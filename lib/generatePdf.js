...
function generatePdf(resDoc) {
  //make a promise that wraps the pandoc call
  return new Promise((resolve, reject) => {
    //start render with pandoc
    pandoc(
      //generate latex from the resolution
      generateLatex(resDoc),

      //build options string to create the right file
      "-o public/rendered/" + resDoc.token +

      //set xelatex engine for unicode support
      ".pdf --verbose --pdf-engine=xelatex --template=public/template.latex",

      //handle result and error if occured
      (err, result) => {
        if (err) {
          reject(err);
        } else {
          ...
          try {
            ...
            pageAmount = parseInt(
              result.match(/AED: lastpage setting LastPage\n\[([0-9]+)\]/)[1], 10);
          } catch (err) {
            console.err("could not parse page amount from pandoc output");
          }

          //resolve promise with found page amount
          resolve(pageAmount);
        }
      }
    );
