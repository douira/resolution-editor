...
function prepareHeader(str, maxLength) {
  ...
  return removeIllagalChars(str).trim().substring(0, maxLength || 45);
}

//names of plenaries by id
const plenaryNames = [false, "General Assembly", "Economic and Social Council"];
...
function generateLatex(resDoc, extData, bookletMode) {
  ...
  if (bookletMode) {
    ...
    if (forumData) {
      ...
      plenaryName = plenaryNames[forumData.plenary];
    } else {
      //warn
      logger.warn(
