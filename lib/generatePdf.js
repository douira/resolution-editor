...
//does one render
function doRenderPass(renderDirPath, renderStringLog, passNumber) {
  //start the render process by spawning the xelatex process
  const xelatexProcess = spawn("xelatex", [
  ...
  return new Promise((resolve, reject) => {
    ...
    xelatexProcess.on("exit", () => {
      ...
      if (["No pages of output", ".pdf may not be valid"].some(str => data.includes(str))) {
        //error log data
        logger.error("pdf render error on pass #" + passNumber + ": " + data, { renderStringLog });
        reject("render error");
      } else {
...
module.exports = (renderDoc, templateType) => co(function*() {
  ...
  const [splitData, extData] = yield [templatePromises[templateType], extDataPromise];

  //generate the latex body to enclose in the template and render
  const latexInner = bookletMode ?
    generateBooklet(renderDoc, extData) : generateLatex(renderDoc, extData, false);
  console.log(latexInner);
...
function prepareClause(str) {
  ...
  str = removeIllagalChars(str, false);
...
function prepareHeader(str) {
  ...
  return removeIllagalChars(str, true).trim().substring(0, 500);
}
...
function getFullName(extData, name) {
...
*/
function generateLatex(resDoc, extData, bookletMode) {
  //unpack resolution
  ...
  if (bookletMode) {
    ...
    if (forumData) {
      ...
      //warn
      logger.warn(
        "Forum called " + obj.resolution.address.forum +
    ...
    headerTexts = {
      ...
      sponsor: prepareHeader(capFirst(getFullName(extData, obj.resolution.address.sponsor.main)))
    };
  }
...
QUESTION OF: &${prepareHeader(capFirst(obj.resolution.address.questionOf))}\\\\\\\\
SPONSOR: &${headerTexts.sponsor}\\\\\\\\`;
  ...
  if (obj.resolution.clauses.operative.length) {
    ...
    str += "\\begin{enumerate}[1., align = left, leftmargin =2cm , " +
...
function generateBooklet(booklet, extData) {
...
\\newgeometry{twoside,headheight=14pt,headsep=12pt,footskip=24pt, vmargin=2cm, includeheadfoot}`;
  ...
  return str + booklet.resolutions.map(resDoc =>
    "\\begin{resolution}" +
    generateLatex(resDoc, extData, true) +
    "\\end{resolution}"
  ).join("");
}
