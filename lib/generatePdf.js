...
[
  ...
  templateDef => ({
    ...
    templatePromise: fs.readFile("./resources/template.latex", "utf8").then(
  ...
  return new Promise((resolve, reject) => {
    ...
    process.on("exit", () => {
      ...
      if (data.includes("No pages of output")) {
        //error log data
        logger.error("pdf render error: " + data);
        reject("render error");
      } else {
...
module.exports = (renderDoc, templateType) => co(function*() {
  ...
  yield fs.writeFile(inputFilePath, splitData[0] + latexInner + splitData[1]);
  ...
  if (! pageAmount) {
    ...
    logger.error("could not parse page amount from renderlog output");
