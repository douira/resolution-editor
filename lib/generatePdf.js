...
\\footnotesize`;
  ...
  str += booklet.signatures
    .map((signature, index) => {
      ...
      if (booklet.signatures.length === 7) {
        //return drawing code for second row signature, with or without offset
        return `\\draw (${index}*\\bottomstrichbreite+${index}*\\luecke${applyOffset},\\zweitezeile) -- (${index +
          1}*\\bottomstrichbreite+${index}*\\luecke${applyOffset},\\zweitezeile);\\node [align=center] at (${index +
          0.5}*\\bottomstrichbreite+${index}*\\luecke${applyOffset},\\zweitezeile-0.3)[below] {${
          signature.name
        }};\\node [align=center] at (${index +
          0.5}*\\bottomstrichbreite+${index}*\\luecke${applyOffset},\\zweitezeile-1)[below] {${
          signature.position
        }};`;
      } else {
        //return drawing code for second row signature, with or without offset
