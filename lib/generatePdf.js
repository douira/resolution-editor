...
\\newcommand{\\sponsor}{${headerTexts.sponsor}}`;
  ...
  if (obj.resolution.clauses.operative.length) {
    ...
    obj.resolution.clauses.operative.map((clause, index, arr) => {
      ...
      return cStr + "\\end{oc}";
    }).join("\n") + "\\end{enumerate}" + (bookletMode ? "\\end{resolution}" : "");
  }
...
*/
function generateTitlePage(booklet) {
...
\\def\\zweitezeile{1}`;
  ...
  booklet.signatures.map((signature, index) => {
    ...
    if (index <= 3) {
      ...
      //reduce index by 4 to get index of this signature in the second row
      index = index - 4;
...
//generates latex to to be rendered for the whole passed resolution booklet
function generateBooklet(booklet) {
  //booklet may onl have 7 or 8 signatures for now
  ...
  //generate latex for booklet header and add latex code for all included resolutions
  return generateTitlePage(booklet.title, booklet.session, booklet.signatures) +
    booklet.resolutions.map(resDoc => generateLatex(resDoc, true)).join("");
}
