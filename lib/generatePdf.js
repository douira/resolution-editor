...
}, () => console.error("Could not read template latex file"));
...
module.exports = function generatePdf(resDoc) { return co(function* () {
  //the directory to do the rendering in
  const renderDirPath = "./render/" + resDoc.token;

  //make the path for the latex input file
  const inputFilePath = renderDirPath + "/input.latex";
  ...
  try {
    ...
    console.error("could not parse page amount from pandoc output");
  }

  //return found page amount
  return pageAmount;

  //remove it when done
  /*yield fs.remove(renderDirPath).catch(
    () => console.error("couldn't remove render directory")
  );*/
}); };
