...
resUtil.checkPermission = (resolution, permission, matchMode) => {
  ...
  */
  ...
  switch (matchMode) {
    ...
    case "save":
      return (
        permission.level === "MA" || //master can override restrictions
        //chair ok in all stages if generally allowed
        ((chairlikePerm ||
          //allow if at active editing state and has Delegate  permission
          (resolution.stage <= 1 && permission.level === "DE") ||
    ...
    case "advance":
      ...
      return (
        ...
        (permission.level === "MA" || //chair ok in all stages if generally allowed
          ((chairlikePerm ||
            ...
            (resolution.stage === 3 && permission.level === "FC") ||
            (resolution.stage === 4 &&
              (permission.level === "FC" || permission.level === "SC"))) &&
            //must be allowed to advance at all
            resolution.attributes !== "noadvance" &&
            resolution.attributes !== "static"))
      );
    case "liveview":
      //always allow MA to do lv
      return (
        permission.level === "MA" || //SG/CH in stage 6 (in debate) and SG in stage 10
...
resUtil.checkStaticPermission = (permission, mode) => {
  ...
  switch (mode) {
    default:
      logger.fatal(
        `invalid match mode ${mode} specified for static permission check!`
      );
      break;
    case "printqueue":
    ...
    case "fcqueue":
      //fc work queue
      return (
        permission.level === "MA" || chairlikePerm || permission.level === "FC"
      );
    case "forum":
      //forum overview
...
const countRecursive = (obj, depth) => {
  ...
  if (obj instanceof Array) {
    ...
    return combineAdd(
      countRecursive(clauses.preambulatory),
...
resUtil.getMetaInfo = resDoc => {
  ...
  if (resDoc.stage) {
    ...
    infoObj.depthIndex = round(
      countInfo.depthIndex / countInfo.clauseCount[0],
      2
    );
    infoObj.avrgClauseWords = round(
      countInfo.words / countInfo.clauseCount[0],
      2
    );
...
function indentConstant(prefix, toIndent, dist, delim) {
  ...
  return (
    prefix +
    delim.repeat(Math.max(dist - prefix.length, 0)) +
...
const pushClauses = (pushInto, clauses, indenters, clauseType, depth) => {
  ...
  clauses.forEach((clause, index) => {
    ...
    let mainContent = hasPhrase
      ? `${clause.phrase}_ ${clause.content}`
      : clause.content;
...
const preparePushClauses = linesArr =>
  pushClauses.bind(null, line => {
    //push onto array
...
resUtil.renderPlaintext = doc => {
  ...
  out.push(
    `Token: ${doc.token}, Stage: #${doc.stage}, FormatVersion: ${
      doc.content.version
    }`
  );
  ...
  out.push(
    `${indentConstant("QUESTION OF:", res.address.questionOf, addressIndent)}\n`
  );
  out.push(
    `${indentConstant("SPONSOR:", res.address.sponsor.main, addressIndent)}\n`
  );
  out.push(
    `${indentConstant(
      "CO-SPONSORS:",
      res.address.sponsor.co.join(", "),
      addressIndent
    )}\n`
  );
  ...
  preparePushClauses(out)(
    ...
    [
      ...
      {
        primary: index =>
          indent(secIUnit + iUnit) +
          indentConstant(
            `${"abcdefghijklmnopqrstuvwxyz"[(index + 1) % 26]})`,
            iUnit
          ),
...
const removeUnsavedResolutions = resolutions => {
  ...
  resolutions
    .deleteMany({
      ...
      changed: {
        //save time must be more than unsavedResolutionTTL ms ago
...
require("../lib/database").fullInit.then(collections => {
  ...
  setInterval(
    removeUnsavedResolutions.bind(null, resolutions),
    removeUnsavedInterval
  );
});
...
resUtil.makeNewThing = (req, res, isToken) =>
  ...
  new Promise(function attempt(resolve, reject) {
    ...
    (isToken
      ? resolutions.findOne({ token: thing })
      : access.findOne({ code: thing })
    ).then(
      document => {
        //if it exists
        if (document) {
      ...
      () => {
        reject(new Error("db read error"));
      }
    );
