...
resUtil.checkStaticPermission = function(permission, mode) {
  //TODO: allow SG perm where applicable
  ...
  switch (mode) {
    ...
    case "fcqueue":
      //fc work queue
      return permission.level === "MA" || permission.level === "CH" || permission.level === "FC";
    case "forum":
      //forum overview
      return permission.level === "MA" || permission.level === "SG" || permission.level === "CH";
  }
};
...
resUtil.validateAccessLevel = function(level) {
  //need to be a string and one of the allowed level names
  return typeof level === "string" && resUtil.validAccessLevels.includes(level);
};
...
resUtil.makeNewThing = function(req, res, isToken) {
