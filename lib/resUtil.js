...
resUtil.checkEditPermissionMatch = function(resolution, permission) {
  /* Permission Levels:
    ...
    SC: Secreteriat (same editing rights as DE, but allowed to advance from 4 to 5)
  ...
  */
  ...
  return permission.level === "CH" || permission.level === "MA" || //chair and admin always ok
    //allow if at active editing state and has delegate or secreteriat permission
...
//checks if given permission code doc allows advancing given resolution
resUtil.checkAdvancePermissionMatch = function(resolution, permission) {
  //check permission and stage conditions
  return permission.level === "CH" || permission.level === "MA" || //chair and admin always ok
...
databaseInterface.onload((loadedDb) => {
  ...
  const resolutions = loadedDb.collection("resolutions");
