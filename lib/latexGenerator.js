...
function generateLatex(obj) {
  ...
  if (obj.resolution.clauses.operative.length) {
    ...
    obj.resolution.clauses.operative.map((clause, index, arr) => {
      ...
      if (subsPresent) {
        ...
        cStr += clause.sub.map((subclause, subIndex, subArr) => {
          ...
          subContent += getOpPunctuation(
            ...
            ! subsubsPresent && lastSubClause && !hasExtContent && lastClause 
          );
          ...
          if (subsubsPresent) {
            ...
            subContent += subclause.sub.map((subSub, subSubIndex, subSubArr) => {
              ...
              return "\\item " +
                prepareClause(subSub) +
                getOpPunctuation(
                  false,
                  lastSubSubClause && lastSubClause && !subHasExtContent,
                  lastSubSubClause && lastSubClause && !subHasExtContent && lastClause
                );
            }).join("\n");
          ...
          if(subclause.contentExt && subclause.contentExt.length){
            subContent += "~"+subclause.contentExt+getOpPunctuation(
              false,
              lastSubClause && !hasExtContent,
              lastSubClause && !hasExtContent && lastClause
            );
          }
      ...
      if(clause.contentExt && clause.contentExt.length){
          cStr += clause.contentExt+getOpPunctuation(false,true,lastClause);
      }
