...
module.exports = (obj) => {
  ...
  let str = `
  \\newcommand{\\forum}{${capFirst(obj.resolution.address.forum)}}
  \\newcommand{\\issue}{${capFirst(obj.resolution.address.questionOf)}}
  \\newcommand{\\sponsor}{${capFirst(obj.resolution.address.sponsor.main)}}
  \\begin{tabbing}
  ...
  str += obj.resolution.clauses.preambulatory.map(
    (clause) => {
      ...
      let str = `\\begin{preamb}{${capFirst(clause.phrase)}}{${
  ...
  if (obj.resolution.clauses.operative.length) {
    ...
    obj.resolution.clauses.operative.map((clause, index, arr) => {
      ...
      let cStr = `\\begin{oc}{${capFirst(clause.phrase)}}{${clause.content +
      ...
      if (subsPresent) {
        ...
        cStr += clause.sub.map((subclause, subIndex, subArr) => {
          ...
          if (isStringType) {
            ...
            subContent += capFirst(subclause);
          } else {
            //add content, punctuation comes later
            subContent += capFirst(subclause.content);
          }
