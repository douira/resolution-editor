...
const attemptDBConnect = (resolve, reject, attemptCount) => {
  ...
  MongoClient.connect(`mongodb://resolutionEditor:${
    credentials.dbUserPassword}@localhost:27017/resolution-editor`, { useNewUrlParser: true }).then(
...
module.exports.fullInit = co(function*() {
  ...
  try {
    yield [
      ...
      db.collection("sessions").createIndex(
        ...
        { expires: 1 }, { expireAfterSeconds: sessionExpireSeconds }
      ),
    ...
    logger.fatal("failed to init collections and indexes", { stack: err.stack });
  }
  ...
  db.collection("resolutionId").insertMany([
    {
      year: currentYear,
      counter: 1
    },
    ...
    writeErrors
      ...
      .forEach(err =>
        logger.fatal("failed to init resolutionId counter document", { stack: err.stack })
      );
  });
