...
function catchIndexCreationError(err) {
  //log error info
  logger.fatal(err, "index creation error");

  //also throw error to terminate, this is bad enough that
...
.then(db => {
  ...
  db.collection("resolutions").createIndex({ token: "text" }, { unique: true })
    ...
    //.then(db.collection("access").deleteMany({}))
    //.then(db.collection("resolutions").deleteMany({}))
    //.then(db.collection("resolutionArchive").deleteMany({}))
    //.then(db.collection("resolutionId").deleteMany({}))
    //.then(db.collection("sessions").deleteMany({}))
    //.catch(err => logger.error(err, "db-emptying error"))
    ...
    .then(collInfos => {
      ...
      .then(
        ...
        err => logger.fatal(err, "mongo resolution id init error"));
    }, err => logger.fatal(err, "mongo list collections error"));
