...
routingUtil.getAndPost = (onRouter, url, handler) =>
  ["get", "post"].forEach(method => onRouter[method](url, handler));
...
routingUtil.checkToken = function(req, res, modifyResolution, callback) {
  ...
  if (tokenProcessor.check(token)) {
    ...
    (doModify //modify as well if flag set
      ? resolutions.findOneAndUpdate({ token }, modifyResolution, {
          returnOriginal: true
        })
...
routingUtil.requireSession = (permMode, req, res, callback) => {
  ...
  if (
    ...
    req.session.code &&
    (permMode === "any" ||
      resUtil.checkStaticPermission(req.session.doc, permMode))
  ) {
    ...
    res.redirect(
      `/session/login?backto=${encodeURIComponent(req.originalUrl)}`
    );
  }
};
