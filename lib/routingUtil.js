...
const routingUtil = module.exports = {
  ...
  //requires session login as admin
  requireSession: (level, req, res, callback) => {
    //normalize level specification: wrap in array if string
    if (typeof level === "string") {
      level = [level];
    } else if (! (typeof level === "object" && level instanceof Array)) {
      //not valid, make empty (admin only access)
      level = [];
    }

    //check for session, specified or admin
    if (req.session &&
        req.session.code &&
        (level.includes(req.session.doc.level) || req.session.doc.level === "MA")) {
      //has session, call callback as user is permitted
      callback(req.session.code);
    } else {
  ...
  fullAuth: (req, res, callback, opts) => {
    ...
    routingUtil.checkToken(req, res, (token, resolutionDoc) => {
      ...
      if (req.body && req.body.code && req.body.code.length || req.session.doc) {
        //check sent code
        routingUtil.checkCode(req, res, codeDoc => {
