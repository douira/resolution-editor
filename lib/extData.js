...
const useData = data => {
  ...
  if (typeof data === "string") {
    try {
      ...
      logger.error("gotten raw extData is invalid json");
      return false;
    }
  }
...
module.exports = co(function*() {
  ...
  try {
    //the server at this url has its own caching so we don't have to worry about its load
    rawData = yield request(
      "http://munol.org/masscontent/reso_editor_data.json"
    );
  } catch (e) {
