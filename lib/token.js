...
//make object to attach functions to
const tokenLib = {};
...
tokenLib.getValidation = (forStr, vLength) => {
  ...
  let hashNumber = XXH.h32(
    forStr + credentials.tokenSuffix,
    credentials.tokenSeed
  ).toNumber();
...
tokenLib.check = checkThis => {
  ...
  return (
    tokenLib.getValidation(
      checkThis.substr(0, randomLength + 1),
      validationLength,
      checkThis[0]
    ) === checkThis.substr(-validationLength)
  );
};
